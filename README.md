...
### Notion DB 一覧ビュー(自動取得した案件が並ぶ)

![Notion 案件一覧](images/notion-database-overview.png)

各行に案件名・取得元(クラウドワークス/ランサーズ/ココナラ)・Claude完結度・スコア・地雷タグが自動入力される。S/A判定のみがここに到達(B+以下は事前ブロック)。

### 取得対象の実案件例(クラウドワークス公開ページ)

![クラウドワークス案件詳細](images/sample-job-detail.png)

`Google NotebookLMを外部操作するPython自動化ツール作成依頼 ¥16,500`
ツールがこの案件のタイトル・本文をスクレイピング→「Python・自動化・Cookie操作」のキーワードヒット→**Claude完結度:確実完結** と判定→ Notion 登録 + Gmail 通知という流れ。
